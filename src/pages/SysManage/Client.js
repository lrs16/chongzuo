...
import { Table, Checkbox, Input, Divider, Select, Button, notification, Form } from 'antd';
...
class Client extends React.Component {
        ...
        renderDom: (form, record) => {
          return (
            <FormItem style={{ margin: 0 }}>
              ...
              })(<CheckboxGroup disabled={record.type === 'view'} options={fruitList} />)}
            </FormItem>
          );
        },
      },
  ...
  addSubmit(form, record) {
    ...
    form.validateFields((error, values) => {
      ...
      values.evaluate = values.evaluate == '1';
  ...
  giveUpUpdata(record) {
    const { data } = this.state;
    const editRow = data.find(item => item.id === record.id);
    editRow.type = 'view';
    this.updateDataSource(data);
  }
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <Button style={{ marginBottom: '10px' }} disabled={isRowOpen} onClick={this.addRow}>
            + 添加
          </Button>
          <Table
