    ...
    <div>
      ...
      <Spin spinning={loadingorder}>
        <Row style={{ marginTop: 16 }}>
          ...
          {(!analysislist || (analysislist && analysislist === undefined)) && (
            ...
            <Row>
              <Col span={6}>
                <StatisticsCard
                  title="故障工单："
                  staticName="故障总数"
                  time1={analysislist.time1}
                  time2={analysislist.time2}
                  value={analysislist.allNum}
                  suffix="单"
                  des="环比"
                  desval={`${analysislist.allRingPoints}%`}
                  type={Number(analysislist.allRingPoints) > 0 ? 'up' : 'down'}
                />
              </Col>
              <Col span={6}>
                <StatisticsCard
                  ...
                  time1={analysislist.time1}
                  time2={analysislist.time2}
                  value={analysislist.closeNum}
                  suffix="单"
                  des="环比"
                  desval={`${analysislist.closeRingPoints}%`}
                  type={Number(analysislist.closeRingPoints) > 0 ? 'up' : 'down'}
                />
              </Col>
              <Col span={6}>
                <StatisticsCard
                  ...
                  time1={analysislist.time1}
                  time2={analysislist.time2}
                  value={analysislist.unCloseNum}
                  suffix="单"
                  des="环比"
                  desval={`${analysislist.unCloseRingPoints}%`}
                  type={Number(analysislist.unCloseRingPoints) > 0 ? 'up' : 'down'}
                />
              </Col>
              <Col span={6}>
                <StatisticsCard
                  title="完成率："
                  value={analysislist.point}
                  suffix="%"
                  des="环比"
                  desval={`${analysislist.ringPoints}%`}
                  type={Number(analysislist.ringPoints) > 0 ? 'up' : 'down'}
                />
              </Col>
            </Row>
          )}
        ...
        <Row gutter={16}>
          {analysislist && (
            <Col span={12} xs={12} style={{ marginTop: 16 }}>
              <div className={styles.statisticscard}>
                <Avatar icon="desktop" />
                <b>系统故障率、可用率</b>
              </div>
              ...
              {analysislist && analysislist !== undefined && (
                <Row>
                  <Col span={8}>
                    <StatisticsCard
                      title="故障时长："
                      value={analysislist.handleTime}
                      suffix="H"
                      des="环比"
                      desval={`${analysislist.handleTimeRingPoints}%`}
                      type={Number(analysislist.handleTimeRingPoints) > 0 ? 'up' : 'down'}
                    />
                  </Col>
                  <Col span={8}>
                    <StatisticsCard
                      title="故障率："
                      value={analysislist.troublePoint}
                      suffix="%"
                      des="环比"
                      desval={`${analysislist.troubleRingPoints}%`}
                      type={Number(analysislist.troubleRingPoints) > 0 ? 'up' : 'down'}
                    />
                  </Col>
                  <Col span={8}>
                    <StatisticsCard
                      title="可用率："
                      value={analysislist.canUserPoint}
                      suffix="%"
                      des="环比"
                      desval={`${analysislist.canUserRingPoints}%`}
                      type={Number(analysislist.canUserRingPoints) > 0 ? 'up' : 'down'}
                    />
                  </Col>
                </Row>
              )}
                ...
                <Row>
                  ...
                  <Col span={8}>
                    <StatisticsCard
                      ...
                      time1={analysislist.time1}
                      time2={analysislist.time2}
                      value={analysislist.isMaster}
                      suffix="单"
                      des="环比"
                      desval={`${analysislist.isMasterRingPoints}%`}
                      type={Number(analysislist.isMasterRingPoints) > 0 ? 'up' : 'down'}
                    />
                  </Col>
                  <Col span={8}>
                    <StatisticsCard
                      ...
                      time1={analysislist.time1}
                      time2={analysislist.time2}
                      value={analysislist.noMaster}
                      suffix="单"
                      des="环比"
                      desval={`${analysislist.noMasterRingPoints}%`}
                      type={Number(analysislist.noMasterRingPoints) > 0 ? 'up' : 'down'}
                    />
                  </Col>
                </Row>
              )}
        ...
        <Row style={{ marginTop: 16 }}>
          ...
          <Col span={8}>
            <Card onMouseDown={() => setPicVal({})}>
              {(!blameconditlist.pieChart ||
                (blameconditlist &&
                  blameconditlist.pieChart &&
                  blameconditlist.pieChart.length === 0)) && <Empty style={{ height: '300px' }} />}
          ...
          <Col span={16}>
            <Card onMouseDown={() => setPicVal({})} style={{ marginLeft: '-1px' }}>
              {(!blameconditlist.lineChart ||
                (blameconditlist &&
                  blameconditlist.lineChart &&
                  blameconditlist.lineChart.length === 0)) && <Empty style={{ height: '300px' }} />}
              {blameconditlist.lineChart && blameconditlist.lineChart.length > 0 && (
                <SmoothLine
                  ...
                  uncheckedname={['总数']}
                  cols={linecols(blameconditlist.lineChart)}
                  lock
                />
              )}
            </Card>
      ...
      <Row style={{ marginTop: 16 }}>
        ...
        <Col span={8}>
          <Card onMouseDown={() => setPicVal({})}>
            ...
            {(!typeconditlist.allPieChart ||
              (typeconditlist &&
                typeconditlist.allPieChart &&
                typeconditlist.allPieChart.length === 0)) && <Empty style={{ height: '300px' }} />}
        ...
        <Col span={16}>
          <Card onMouseDown={() => setPicVal({})} style={{ marginLeft: '-1px' }}>
            ...
            {(!typeconditlist.allLineChart ||
              (typeconditlist &&
                typeconditlist.allLineChart &&
                typeconditlist.allLineChart.length === 0)) && <Empty style={{ height: '300px' }} />}
            {typeconditlist.allLineChart && typeconditlist.allLineChart.length > 0 && (
              <SmoothLine
                ...
                timeType={values.type}
                cols={linecols(typeconditlist.allLineChart)}
              />
            )}
          </Card>
        </Col>
        <Col span={8}>
          <Card onMouseDown={() => setPicVal({})}>
            ...
            {(!typeconditlist.hardwarePieChart ||
              (typeconditlist &&
                typeconditlist.hardwarePieChart &&
                typeconditlist.hardwarePieChart.length === 0)) && (
        ...
        <Col span={16}>
          <Card onMouseDown={() => setPicVal({})} style={{ marginLeft: '-1px' }}>
            ...
            {(!typeconditlist.hardwareLineChart ||
              (typeconditlist &&
                typeconditlist.hardwareLineChart &&
                typeconditlist.hardwareLineChart.length === 0)) && (
              <Empty style={{ height: '300px' }} />
            )}
            {typeconditlist &&
              typeconditlist.hardwareLineChart &&
              typeconditlist.hardwareLineChart.length > 0 && (
                <SmoothLine
                  ...
                  uncheckedname={['总数']}
                  timeType={values.type}
                  cols={linecols(typeconditlist.hardwareLineChart)}
                />
              )}
          </Card>
        </Col>
        <Col span={8}>
          <Card onMouseDown={() => setPicVal({})}>
            ...
            {(!typeconditlist.softPieChart ||
              (typeconditlist &&
                typeconditlist.softPieChart &&
                typeconditlist.softPieChart.length === 0)) && <Empty style={{ height: '300px' }} />}
            {typeconditlist &&
              typeconditlist.softPieChart &&
              typeconditlist.softPieChart.length > 0 && (
                <DonutPCT
        ...
        <Col span={16}>
          <Card onMouseDown={() => setPicVal({})} style={{ marginLeft: '-1px' }}>
            ...
            {(!typeconditlist.softLineChart ||
              (typeconditlist &&
                typeconditlist.softLineChart &&
                typeconditlist.softLineChart.length === 0)) && (
              <Empty style={{ height: '300px' }} />
            )}
            {typeconditlist &&
              typeconditlist.softLineChart &&
              typeconditlist.softLineChart.length > 0 && (
                <SmoothLine
                  ...
                  uncheckedname={['总数']}
                  timeType={values.type}
                  cols={linecols(typeconditlist.softLineChart)}
                />
              )}
          </Card>
        </Col>
      ...
      <Row style={{ marginTop: 16 }}>
        ...
        <Col span={8}>
          <Card onMouseDown={() => setPicVal({})}>
            {(!modelconditlist.pieChart ||
              (modelconditlist &&
                modelconditlist.pieChart &&
                modelconditlist.pieChart.length === 0)) && <Empty style={{ height: '300px' }} />}
        ...
        <Col span={16}>
          <Card onMouseDown={() => setPicVal({})} style={{ marginLeft: '-1px' }}>
            {(!modelconditlist.lineChart ||
              (modelconditlist &&
                modelconditlist.lineChart &&
                modelconditlist.lineChart.length === 0)) && <Empty style={{ height: '300px' }} />}
            {modelconditlist && modelconditlist.lineChart && modelconditlist.lineChart.length > 0 && (
              <SmoothLine
                ...
                uncheckedname={['总数']}
                timeType={values.type}
                cols={linecols(modelconditlist.lineChart)}
              />
            )}
          </Card>
        </Col>
      ...
      <Row gutter={12} style={{ marginTop: 16 }}>
        <Col span={12}>
          ...
          <Card onMouseDown={() => setPicVal({})}>
            {(!timeoutconditlist || (timeoutconditlist && timeoutconditlist.length === 0)) && (
              <Empty style={{ height: '364.5px' }} />
            )}
        ...
        <Col span={12}>
          ...
          <Card onMouseDown={() => setPicVal({})}>
            {(!analysisreportlist || (analysisreportlist && analysisreportlist.length === 0)) && (
              <Empty style={{ height: '364.5px' }} />
            )}
      ...
      <Row gutter={12} style={{ marginTop: 16 }}>
        <Col span={12}>
          ...
          <ChartCard contentHeight={350} onMouseDown={() => setPicVal({})}>
            {(!handlerlist || (handlerlist && handlerlist.length === 0)) && (
        ...
        <Col span={12}>
          ...
          <ChartCard contentHeight={350} onMouseDown={() => setPicVal({})}>
            {(!registeruserlist || (registeruserlist && registeruserlist.length === 0)) && (
              <Empty style={{ height: '350px' }} />
            )}
