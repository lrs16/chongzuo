...
import Handle from './components/Handle2';
import Registrat from './components/Registrat2';
...
function Registration(props) {
  ...
  const { dispatch, loading, userinfo, tabnew, location, tabdata, errstutas } = props;
  ...
  useEffect(() => {
    if (location.state) {
      if (location.state.cache) {
        const values = RegistratRef.current.Formvalues();
        console.log(values)
        // const values = {};
        const main = {
        ...
        if (show) {
          const v = HandleRef.current.getFieldsValue();
          const handle = {
...
export default connect(({ itsmuser, eventregist, viewcache, loading }) => ({
  tabnew: viewcache.tabnew,
  tabdata: viewcache.tabdata,
  userinfo: itsmuser.userinfo,
  errstutas: eventregist.errstutas,
