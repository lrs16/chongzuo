...
function Besolveddetail(props) {
  ...
  const solvingDisbled = () => {
    if (currntStatus === 29) {
      console.log('currntStatus === 29: ', currntStatus === 29);
      showEdit = true;
      receiveOrders = false;
    }
  };
  ...
  const handleSubmit = params2 => {
    ...
    switch (currntStatus) {
      case 5:
        ...
        saveRegister(circulationSign, params2);
        break;
      case 25:
        circulationSign = currntStatus;
        savePrevies(params2);
  ...
  const saveRegister = (circulationSign, params2) => {
      ...
      if (!err) {
        ...
        return dispatch({
          ...
          if (res.code === 200) {
            ...
            console.log('circulationSign: ', circulationSign);
            if (params2) {
              gotoCirapi();
  ...
  const saveHandle = circulationSign => {
    validateFields((err, values) => {
      const saveData = values;
      ...
      if (!err) {
        ...
        saveApi(saveData, circulationSign);
      }
    });
  };

  const saveConfirm = circulationSign => {
    validateFields((err, values) => {
      const addDateZero = num => {
        return num < 10 ? '0' + num : num;
      };
  ...
  const saveClose = circulationSign => {
    validateFields((err, values) => {
      if (!err) {
        const saveData = values;
        saveData.taskId = id;
  ...
  return (
    <PageHeaderWrapper title={pagetitle}>
        ...
        extra={
          <>
            <>
              ...
              > */}
              {activekey === '1' && !queryStatue && receiveOrders === true && (
                <Button
                  type="primary"
                  style={{ marginRight: 8 }}
      ...
      {/* 编辑页 */}
      {/* 问题登记 */}
      {activekey === '1' && currntStatus === 5 && !queryStatue && (
        <>
                  ...
                  <Col span={8}>
                    <Form.Item label="登记时间">
                      {getFieldDecorator('registerTime', {
                        ...
                        initialValue: todoDetail ? moment(todoDetail.register.registerTime) : '',
                  ...
                  <Col span={8}>
                    <Form.Item label="上传附件">
                      {getFieldDecorator(
                        ...
                        <Upload {...props} fileList={filelist} onChange={uploadonChange}>
                          <Button type="primary">
                            <Icon type="upload" />
                            上传附件
                          </Button>
      ...
      {/* 问题审核 */}
      {activekey === '1' && (currntStatus === 9 || currntStatus === 25) && !queryStatue && (
        <>
          <Collapse
                  ...
                  <Col span={22}>
                    <Form.Item label="审核时间" {...forminladeLayout}>
                      {getFieldDecorator('checkTime', {
                        initialValue: check ? moment(check.checkTime) : '',
                      })(<DatePicker showTime />)}
                    </Form.Item>
                  </Col>
      ...
      {/* 问题处理 */}
      {activekey === '1' && (currntStatus === 29 || currntStatus === 45) && !queryStatue && (
        <>
          <Collapse
            expandIconPosition="right"
            style={{ backgroundColor: 'white', marginTop: '20px' }}
                  ...
                  <Col span={8}>
                    <Form.Item label="处理完成时间">
                      {getFieldDecorator('handleTime', {
                        ...
                        initialValue: handle ? moment(handle.handleTime) : '',
                      })(<DatePicker showTime format="YYYY-MM-DD HH:mm" disabled={showEdit} />)}
                    </Form.Item>
                  </Col>
                  <Col span={8}>
                    <Form.Item label="处理结果">
                      {getFieldDecorator('handleResult', {
                        ...
                        <Select onChange={handleChange} disabled={showEdit}>
                          <Option value="根本解决">根本解决</Option>
                          <Option value="替代解决">替代解决</Option>
                          <Option value="需要发布">需要发布</Option>
                          <Option value="误报">误报</Option>
                          <Option value="自动恢复">自动恢复</Option>
                        </Select>,
                      )}
                    </Form.Item>
                  </Col>
      ...
      {/* 问题确认 */}
      {activekey === '1' &&
        currentProcess === '问题确认' && (currntStatus === 65 || currntStatus === 49) &&
        !queryStatue && (
          <>
            <Collapse
              expandIconPosition="right"
                    ...
                    <Col span={8}>
                      <Form.Item label="确认时间">
                        {getFieldDecorator('confirmTime', {
                          ...
                          initialValue: confirm ? moment(confirm.confirmTime) : '',
                        })(<DatePicker showTime />)}
                      </Form.Item>
                    </Col>
      ...
      {activekey === '1' && currentProcess === '确认会签' && currntStatus === 65 && !queryStatue && (
        <>
          <Collapse
            expandIconPosition="right"
            style={{ backgroundColor: 'white', marginTop: '20px' }}
      ...
      {/* 问题关闭 */}
      {activekey === '1' && (currntStatus === 69 || currntStatus === 85) && !queryStatue && (
        <>
          <Collapse
            expandIconPosition="right"
            style={{ backgroundColor: 'white', marginTop: '20px' }}
