...
function Associateworkorder(props) {
  ...
  return (
    <div>
      <Form layout="inline">
        {chooseClass === 1 && (
          <Form.Item label="事件编号">{getFieldDecorator('eventNumber', {})(<Input />)}</Form.Item>
        )}
        ...
        <Form.Item>
          <Associatedevent chooseClass={chooseClass}>
            <Button type="primary">关联工单</Button>
          </Associatedevent>
        </Form.Item>
      </Form>

      {chooseClass === 1 && <Table columns={columns} dataSource={eventtableList.data} />}

      {chooseClass === 2 && <Table columns={columnRealse} dataSource={realsetableList.data} />}
    </div>
  );
}
