...
const TopnAnalysis = [
  {
    ...
    措施:
    冗余数据、存量数据帮助用户进行后台维护。修改关键字段可以与营销运维人员协商，在营销系统后台批量修改数据的时候更新操作时间字段，XX通过接口同步；`
  },
  {
    field1: '2',
    field2: '档案',
  ...
  },
  {
    ...
    field4: '',
    field5: `原因：
    基本上都是用户营销维护档案不规范，主要字段为版本规约、测量点号和通信地址等，导致推送到中间库后XX同步不了；
    措施：
  ...
  },
  {
    ...
    （2）召测日冻结时间错误，XX档案正常，召测档案查看终端内费率为4正常，查看日冻结的报文发现终端回的上行报文的费率错误无法正常解析。现场处理咨询终端厂家。
    （3）日冻结可以回数，前台显示正常。
  ...
  },
  {
    ...
    （4）可以抄表还在未覆盖，缺少XX点电能表关系，需去营销维护XX点电能表关系。
    （5）刚归档的，出现在前几天抄表失败，系统每天会自动重算前两天的指标，如果没有数就会在抄表失败。
    （6）营销已完成建档，XX还在未覆盖，由于终端规约错误，无法同步到XX。`
  },
  {
    field1: '8',
    field2: '接口',
    ...
    （2）终端（规约版本号、用户编号为空、XX点编号为空、运行状态超出同步范围、标识冲突）；
    （3）采集关系（采集对象运行标识、采集对象用户编号为空）；
    （4）采集关系采集对象运行标识与运行电能表的运行电能表标识不一致；
    （5）因线路台区有多条线路导致用户在营销系统与XX系统所关联的线路不一致
    措施：
    （1）电表或者终端参数错误，建议客户新增先检查档案参数是否正确，在走单；
    （2）对于终端（用户编号为空、XX点编号为空），采集关系（采集对象运行标识、采集对象用户编号为空），建议营销后台整改；
  ...
  },
  {
    ...
    措施：
    （1）维护营销档案、删除手动模型。
    （2）新装更换终端或电表，导致月电量没有需找XX中心李捷重算月电量；若是日、月电量没有需找南瑞重算电量；重算线损。`
  },
  {
    field1: '11',
    field2: '高级功能',
            ...
            <Col span={24}>
              ...
              >
                {getFieldDecorator('name', {
                  ...
                  (
                    <Input style={{ width: 700 }} placeholder={`省级集中XXXX系统软件运维${reporttype === 'week' ? '周' : '月'}报`} />
                  )}
              </Form.Item>
            </Col>
...

            {
              initial && loading === false && startTime && (
                <>
                  ...
                  <Col span={24}>
                    <p>(2)计划{startTime}至{endTime},XXXX系统开展 次发布变更（消缺），变更内容如下</p>
                  </Col>
