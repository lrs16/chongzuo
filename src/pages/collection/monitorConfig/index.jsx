...
import { Button, Divider, Dropdown, Menu, message, Row, Col, Drawer, Tag, Modal } from 'antd';
const { SubMenu } = Menu;
const { confirm } = Modal;
...
import { queryRule, updateRule, addRule, removeRule, querySort } from './service';
...
const handleAdd = async fields => {
  ...
  let interFace = [];
  fields.agents && interFace.push({type: 1, detail: fields.agents});
  fields.snmps && interFace.push({type: 2, detail: fields.snmps});
  fields.jxms && interFace.push({type: 4, detail: fields.jxms});
  fields.ipmis && interFace.push({type: 3, detail: fields.ipmis});
  ...
  const columns = [
    {
      ...
      render: (text, record, index) => {
        return (
          <Button type='link' 
            onClick={() => { setConfigData(record);handleModalVisible(true)} }>
            {text}
          </Button>
        );
      }, 
    ...
    },
    {
      ...
      render: row => 
        row.map(key => {
          ...
          return ( <Tag key={key.typeName} color={color}>{key.typeName}</Tag> )
        }),
    },
    {
      title: 'agent加密',
    ...
    <PageHeaderWrapper>
      <Row>
        <Col span={6}>
          ...
          >
            {menuList && menuList.map((n,index) =>  
                <SubMenu key={index} title={n.sortName}>
                    {n.cheld && n.cheld.map((i,x) => 
                        <Menu.Item key={i.id} >{i.sortName}</Menu.Item>
                    )}
                </SubMenu>
            )}
          </Menu>
        ...
        <Col span={18}>
          <ProTable
            ...
            toolBarRender={(action, { selectedRows }) => [
              <Button type="primary" onClick={() => {setConfigData();handleModalVisible(true)}}>
                添加
              </Button>,
