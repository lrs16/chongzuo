...
const CreateForm = props => {
  ...
  const [jmxVisible, setJmxVisible] = useState(data !== undefined);
        ...
        <Col span={8}>
          <FormItem label="默认">
            {form.getFieldDecorator(`${list}[${key}].main`, {
              ...
              <Radio.Group
                onChange={() => {
                  form.getFieldValue(list).map((_, i) => {
                    if (index !== key) {
                      return form.setFieldsValue({ [`${list}[${i}].main`]: 0 });
                    }
                    return {};
                  });
                }}
            ...
            {param.length > 1 && (
              <Button type="link" onClick={() => onClick(key)}>
                移除
              </Button>
            )}
          </FormItem>
  ...
  const sortSelect =
    ...
    sorts.map((value, index) => (
      <OptGroup key={index} label={value.sortName}>
        {value.cheld.map(v => (
          <Option key={v.id} value={v.monitorGroupId}>
            {v.monitorGroupName}
          </Option>
        ))}
      </OptGroup>
      ...
      <Tabs defaultActiveKey="1">
        <Tabs.TabPane tab="基本信息" key="1">
          <Form>
              ...
              <Row gutter={16}>
                ...
                <Col span={8}>
                  <FormItem label="监控分类">
                    ...
                    })(<Select mode="tags">{sortSelect}</Select>)}
                  </FormItem>
                </Col>
                <Col span={8}>
                  <FormItem label="描述">
                  ...
                  <FormItem label="监控方式/协议">
                    {form.getFieldDecorator('interFace', {
                      ...
                      <Checkbox.Group>
                        <Checkbox
                          value="agent"
                          onChange={() => setAgent({ ...agent, visible: !agent.visible })}
                        >
                          agent代理程序接口
                        </Checkbox>
            ...
            <br />
            {agent.visible && (
              <Card title="agent代理程序接口">
                <Button
                  type="primary"
                  onClick={() => setAgent({ ...agent, keys: agent.keys.concat(agentNumber + 1) })}
                >
                  添加
                </Button>
            ...
            {snmpVisible && (
              <Card title="SNMP接口">
                ...
                <Button type="primary" onClick={() => setSnmpKeys(snmpKeys.concat(snmpNumber + 1))}>
                  添加
                </Button>
                {create(snmpKeys, 'snmps', k => {
                  setSnmpKeys(snmpKeys.filter(key => key !== k));
                })}
              </Card>
            )}
            <br />
            {jmxVisible && (
              <Card title="JMX接口">
                <Button type="primary" onClick={() => setJmxKeys(jmxKeys.concat(jmxNumber + 1))}>
                  添加
                </Button>
                {create(jmxKeys, 'jxms', k => {
                  setJmxKeys(jmxKeys.filter(key => key !== k));
                })}
              </Card>
            )}
            <br />
            {ipmiVisible && (
              <Card title="IPMI接口">
                ...
                <Button type="primary" onClick={() => setIpmiKeys(ipmiKeys.concat(ipmiNumber + 1))}>
                  添加
                </Button>
        ...
        <Tabs.TabPane tab="模板信息" key="2">
          <Form>
            <Card>
              <FormItem label="模板名称">
                <Select mode="multiple">{sortSelect}</Select>
              </FormItem>
            </Card>
          </Form>
        </Tabs.TabPane>
...
export default Form.create({
  mapPropsToFields(props) {
    ...
    if (data.jmxs.length) {
      jmxNumber = data.jmxs.length;
      interFace.push('jmx');
      forArrary(data.jmxs, 'jmxs');
    }
