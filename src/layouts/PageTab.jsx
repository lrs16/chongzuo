...
class PageTab extends Component {
  ...
  getData = () => {
    const { tabs, children, user } = this.props;
    const { menuData } = user;
    const { pathname } = tabs;
    ...
    if (!pages.some(page => page.key === pathname)) {
      ...
      myPage.push({ key: pathname, title: name, content: children });
    }
    const keys = {};
    myPage.forEach(item => {
      const { key } = item;
